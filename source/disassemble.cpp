...
class Disassembler {
 public:
  Disassembler(uint32_t options, NameMapper name_mapper, uint32_t extend_indent)
      : print_(spvIsInBitfield(SPV_BINARY_TO_TEXT_OPTION_PRINT, options)),
        ...
        instruction_disassembler_(out_.get(), options, name_mapper, extend_indent),
...
std::string spvInstructionBinaryToText(const spv_target_env env,
  ...
  // Now disassemble!
  Disassembler disassembler(options, name_mapper, extend_indent);
...
spv_result_t spvBinaryToText(const spv_const_context context,
  ...
  // Now disassemble!
  spvtools::Disassembler disassembler(options, name_mapper, extend_indent);
  if (auto error =
